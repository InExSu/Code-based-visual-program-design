      ...
      while (state !== State.COMPLETE && state !== State.ERROR) {
        ...
        case State.RUNNING: {
          ...
          const indent =
              deps[node].length === 0
                ? 0
                : Math.max(...deps[node].map(p => indentMap.get(p) || 0), 0) +
                  1
          indentMap.set(node, indent)

          log(
            `▶ #${order} START: ${node} [+${Date.now() - startTime}ms]`,
            colors.start,
            indent,
          )
          ...
          try {
            ...
            if (!result?.info || result.info.trim() === '') {
              ...
              const resultInfo = logConfig.showResults
                ? ` → info: "${result.info}"`
                : ''
              log(
                `✓ COMPLETE: ${node} (${duration}ms)${resultInfo}`,
                colors.complete,
                indent,
              )
            }
